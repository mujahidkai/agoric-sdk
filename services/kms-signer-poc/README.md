...
```sh
gcloud kms keyrings create agoric --location "$REGION"
```
...
gcloud kms keys create wallet0 \
  --keyring agoric --location "$REGION" \
...
```
projects/<YOUR_PROJECT_ID>/locations/<REGION>/keyRings/agoric/cryptoKeys/wallet0/cryptoKeyVersions/1
```

You can confirm the version exists and is ready with:

```sh
gcloud kms keys versions list --key wallet0 --keyring agoric --location "$REGION"
gcloud kms keys versions list --key wallet1 --keyring agoric --location "$REGION"
```
...
```yaml
KMS_KEY_VERSIONS: "projects/<YOUR_PROJECT_ID>/locations/us-central1/keyRings/agoric/cryptoKeys/wallet0/cryptoKeyVersions/1,projects/<YOUR_PROJECT_ID>/locations/us-central1/keyRings/agoric/cryptoKeys/wallet1/cryptoKeyVersions/1"
RPC: "<RPC_URL>"
```
