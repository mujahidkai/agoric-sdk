...
1. Call `KeyManagementServiceClient.createCryptoKey` with purpose
   `ASYMMETRIC_SIGN` and algorithm `EC_SIGN_SECP256K1_SHA256` (or
   `createCryptoKeyVersion` on an existing key). KMS mints a fresh,
   non-exportable secp256k1 keypair inside the HSM.
